...
plot.sparsetMod <- function(object, y="expobject", lb=0, ub=Inf, box=TRUE, color="overlapMu", ...) {
  ...
  if(y=="accuracy"){myYlab <- "Accuracy [cor(true,estimated)]"}else if(y=="expobject"){myYlab <- "Expected genetic object (accuracy and intensity relationship)"}else{myYlab <- y}
